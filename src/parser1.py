...
from nn_modules import InputLayer
import time
...
class Parser(object):
    def __init__(self, conf, label_dict_size):
        self._conf = conf
        ...
        if self._use_cuda:
            ...
            self._cuda_device = 0
        # self._label_embed = nn.Embedding(label_dict_size, conf.label_emb_dim, padding_idx=padding_id).cuda(self._cuda_device)
        # label_emb_init = np.random.randn(label_dict_size, conf.label_emb_dim).astype(data_type)  # normal distribution
        # self._label_embed.weight.data = torch.from_numpy(label_emb_init)
        ...
        self.src_label_embed = nn.Embedding(label_dict_size, conf.src_label_emb_dim, padding_idx=padding_id).cuda(self._cuda_device)
        self._domain_batch = torch.arange(5).cuda(self._cuda_device)
        # self._treelstm = BiTreeLSTM(self._conf.lstm_hidden_dim * 2 + self._conf.src_label_emb_dim, self._conf.treelstm_hidden_dim, self._conf.inside_treelstm_dropout_ratio).cuda(self._cuda_device)
        self._treelstm = BiTreeLSTM(self._conf.lstm_hidden_dim * 2, self._conf.treelstm_hidden_dim, self._conf.inside_treelstm_dropout_ratio).cuda(self._cuda_device)
        self._treegru = BiTreeGRU(self._conf.lstm_hidden_dim * 2 + self._conf.src_label_emb_dim, self._conf.treelstm_hidden_dim).cuda(self._cuda_device)
    ...
    def init_models(self):
        ...
        self._input_layer = InputLayer('input', self._conf, self._word_dict.size(), self._ext_word_dict.size(),\
                self._char_dict.size(), self._tag_dict.size(), self._label_dict.size(), self._ext_word_emb_np, \
                bert_path, bert_dim, bert_layer)
        
        bert_vocab_path = "../xlm-roberta-base/"
        ...
        if self._conf.is_multi:
            for i in range(self._conf.domain_size):
                self.add_mlp_biaffine(i, 2 * self._conf.lstm_hidden_dim + 2 * self._conf.treelstm_hidden_dim)
        else:
            self.add_mlp_biaffine(1, 2 * self._conf.lstm_hidden_dim + 2 * self._conf.treelstm_hidden_dim)
    ...
    def forward(self, words, ext_words, tags, masks, domains, domain_id, word_lens, chars_i, wordbert, src_heads, src_labels, unlabel=False):
        is_training = self._input_layer.training

        subword_idxs, subword_masks, token_starts_masks, word_idxs = [], [], [], []
        for e in wordbert:
            subword_ids, mask, token_starts = self.bertvocab.subword_tokenize_to_ids(e)
            subword_ids = subword_ids.cuda(self._cuda_device)
            mask = mask.cuda(self._cuda_device)
            token_starts = token_starts.cuda(self._cuda_device)
            token_starts[[0, -1]] = 0
        ...
        src_labels[src_labels == -1] = 0
        x_src_label_embed = self.src_label_embed(src_labels.long()).transpose(0, 1).cuda(self._cuda_device)
        if is_training:
            x_src_label_embed = drop_sequence_shared_mask(x_src_label_embed, self._conf.src_label_dropout_ratio)

        # treelstm_input = torch.cat((lstm_out, x_src_label_embed), dim=2)
        treelstm_input = lstm_out
        # treegru_input = torch.cat((lstm_out, x_src_label_embed), dim=2)
        _, treelstm_out = self._treelstm(treelstm_input, src_heads)
        # treegru_out = self._treegru(treegru_input, src_heads)
        if is_training:
            treelstm_out = drop_sequence_shared_mask(treelstm_out, self._conf.outside_treelstm_dropout_ratio)
            # treegru_out = drop_sequence_shared_mask(treegru_out, self._conf.outside_treelstm_dropout_ratio)

        mlp_input = torch.cat((lstm_out, treelstm_out),dim=2)  # ud数据集直接使用lstm_out

        if self._conf.is_multi:
            arc_scores, label_scores = self.mlp_biaffine_module(domain_id, mlp_input, is_training)
        else:
            arc_scores, label_scores = self.mlp_biaffine_module(1, mlp_input, is_training)
    ...
    def mlp_biaffine_module(self, domain_id, mlp_input, is_training):
        # 获取列表的长度
        mlp_layer_length = len(self._mlp_layer)


        mlp_out = self._mlp_layer[domain_id-1](mlp_input)
        if is_training:
    ...
    def compute_loss(arc_scores, label_scores, gold_arcs, gold_labels, total_word_num, one_batch):
        ...
        # 将 gold_arcs 和 gold_labels 转换为长整型
        gold_arcs = gold_arcs.to(torch.long)
        gold_labels = gold_labels.to(torch.long)
    ...
    def train(self):
        ...
        ud, vndt = self._train_datasets[0].batch_num, self._train_datasets[1].batch_num
        domain, domain_src, domain_tgt, train_iter, udomain, udomain_src, udomain_tgt, u_cnt, l_cnt = 0, 0, 0, 0, 0, 0, 0, 0, 0
        dataset_names = ["ud", "vndt"]  # 用名称表示每个数据集
        while True:
            if domain == 0:
                dataset_name = "ud"
                ...
                dataset_name = "vndt"
                dataset = self._train_datasets[1]
    ...
    def train_or_eval_one_batch(self, dataset, is_training, unlabel = False, eval_iter = -1):
        one_batch, total_word_num, max_len = dataset.get_one_batch(rewind=is_training)#ng)
        # NOTICE: total_word_num does not include w_0
        if len(one_batch) == 0:
            print("one_batch is none" + dataset.file_name_short)
            return 0, None
        if unlabel == False:
            words, ext_words, tags, gold_heads, gold_labels, lstm_masks, domains, domains_nadv, word_lens, chars_i, wordbert, src_labels, src_heads = \
                    self.compose_batch_data_variable(one_batch, max_len)
            arc_scores, label_scores = self.forward(words, ext_words, tags, lstm_masks, domains, dataset.domain_id, word_lens, chars_i, wordbert, src_labels, src_heads)
            ...
            for i in range(1, inst.size()):
                ...
                if unlabel == False:
                    if inst.heads_i[i] != ignore_id_head_or_label:
                        label_dict.add_key_into_counter(inst.src_labels_s[i])
            ...
            for i in range(0, inst.size()):
                ...
                if unlabel == False:
                    ...
                    if inst.src_heads_i[i] != ignore_id_head_or_label:
                        inst.src_labels_i[i] = label_dict.get_id(inst.src_labels_s[i])
    ...
    def del_model(path, eval_num):
        path = os.path.join(path, './../src/models-%d/' % eval_num)
    ...
    def load_model(self, path, eval_num):
        print("jjj", eval_num)
        path = os.path.join(path, './../src/models-%d/' % 158 )
        assert os.path.exists(path)
            ...
            for inst in this_bucket:
                ...
                if unlabel == False:
                    inst.words_i, inst.ext_words_i, inst.tags_i, inst.heads_i, inst.labels_i, inst.lstm_mask, inst.domains_i,\
                            inst.domains_nadv_i, inst.word_lens, inst.chars_i, inst.words_s,  inst.src_heads_i,inst.src_labels_i  = self.pad_one_inst(inst, max_len)
                else:
    ...
    def compose_batch_data_variable(self, one_batch, max_len, unlabel=False):
        words, ext_words, tags, heads, labels, lstm_masks, domains, domains_nadv, wordbert, src_heads, src_labels = [], [], [], [], [], [], [], [], [], [], []
        chars_i = None
        i=0
        for inst in one_batch:
            if i == 0:
        ...
        if unlabel == False:
            words, ext_words, tags, heads, labels, lstm_masks, domains, domains_nadv, word_lens, chars_i, src_heads, src_labels = \
                torch.from_numpy(np.stack(words, axis=0)), torch.from_numpy(np.stack(ext_words, axis=0)), \
                    ...
                    torch.from_numpy(np.stack(src_heads, axis=0)), torch.from_numpy(np.stack(src_labels, axis=0))
        else:
        ...
        if self._use_cuda:
            if unlabel==False:
                words, ext_words, tags, heads, labels, lstm_masks, domains, domains_nadv, word_lens, chars_i, src_heads, src_labels = \
                        ...
                        src_heads.cuda(self._cuda_device), src_labels.cuda(self._cuda_device)
            else:
                words, ext_words, lstm_masks, domains, domains_nadv, word_lens, chars_i, tags = \
                        words.cuda(self._cuda_device), ext_words.cuda(self._cuda_device), \
        ...
        if unlabel == False:
            return words, ext_words, tags, heads, labels, lstm_masks, domains, domains_nadv, word_lens, chars_i, wordbert, src_heads, src_labels
        else:
            return words, ext_words, lstm_masks, domains, domains_nadv, word_lens, chars_i, tags, wordbert

    # def _label_embed(self, label_dict_size):
    #     self._label_embed = nn.Embedding(label_dict_size, self._conf.label_emb_dim, padding_idx=padding_id)
    #     label_emb_init = np.random.randn(label_dict_size, self._conf.label_emb_dim).astype(data_type)  # normal distribution
    #     self._label_embed.weight.data = torch.from_numpy(label_emb_init)
