...
class UnInstance(object):
    ...
    @staticmethod
    def compose_sent(words_s, tags_s, heads_i, labels_s, src_heads_i, src_labels_s):
        n1 = len(words_s)
        assert((n1,)*5 == (len(tags_s), len(heads_i), len(labels_s), len(src_heads_i), len(src_labels_s)))
        lines = [''] * (n1 - 1)
        for i in np.arange(1, n1):
            lines[i-1] = ("%d\t%s\t%s\t%s\t%s\t%s\t%d\t%s\t%s\t%s\n" %
                          (i, words_s[i], "_",
                           ...
                           src_heads_i[i], src_labels_s[i]))
        return lines

    def write(self, out_file):
        lines = Instance.compose_sent(self.words_s,self.tags_s, self.heads_i_predict, self.labels_s_predict, self.src_heads_i, self.src_labels_s)
        for line in lines:
            out_file.write(line)
        out_file.write('\n')
