...
class BiTreeGRU(nn.Module):
    def __init__(self, input_size, hidden_size):
        super(BiTreeGRU, self).__init__()
        self._input_size = input_size
        self._hidden_size = hidden_size


        self.dt_treegru = DTTreeGRU(input_size, hidden_size)
        self.td_treegru = TDTreeGRU(input_size, hidden_size)

    def forward(self, inputs, heads):
        max_length, batch_size, size = inputs.size()
        assert size == self._input_size
        trees = []
        indexes = np.zeros((max_length, batch_size), dtype=np.int32)

        for b, head in enumerate(heads):
            root, tree = creatGRUTree(head)
            root.traverse()
            for step, index in enumerate(root.order):
                indexes[step, b] = index
            trees.append(tree)

        dt_outputs, _ = self.dt_treegru(inputs, indexes, trees)
        td_outputs, _ = self.td_treegru(inputs, indexes, trees)
        return torch.cat((dt_outputs, td_outputs), dim=2)
...
class DTTreeGRU(nn.Module):
    ...
    def forward(self, inputs, indexes, trees):
        ...
        for b, length in enumerate([len(tree) for tree in trees]):
            default_tensor = torch.zeros(self._hidden_size, device='cuda:0')
            output = [dt_state[b].get(idx, torch.zeros(self._hidden_size, device='cuda:0')) for idx in range(0, length)]
            output.extend([Var(inputs.data.new(self._hidden_size, device='cuda:0').fill_(0.))] * (max_length - length))
            # outputs = [tensor.to('cuda:0') for tensor in outputs]
            # output_t = [tensor.to('cuda:0') for tensor in output_t]
            outputs.append(torch.stack(output, 0))
            output_t.append(Var(inputs.data.new(self._hidden_size).fill_(0.)))
        return torch.stack(outputs, 0).transpose(0, 1), torch.stack(output_t, 0).transpose(0, 1)
...
class TDTreeGRU(nn.Module):
    ...
    def forward(self, inputs, indexes, trees):
        ...
        for b, length in enumerate([len(tree) for tree in trees]):
            default_tensor = torch.zeros(self._hidden_size, device='cuda:0')
            output = [td_state[b].get(idx, torch.zeros(self._hidden_size, device='cuda:0')) for idx in range(0, length)]
            output.extend([Var(inputs.data.new(self._hidden_size, device='cuda:0').fill_(0.))] * (max_length - length))
            outputs.append(torch.stack(output, 0))
            output_t.append(Var(inputs.data.new(self._hidden_size).fill_(0.)))
...
if __name__ == '__main__':
    ...
    treegru = DTTreeGRU(10, 20)

    print("test node forward: ")
    input = torch.randn((5, 10))
    left_child_h = torch.randn((5, 20))
    right_child_h = torch.randn((5, 20))
    rep = treegru.node_forward(input, left_child_h, right_child_h)
    print(rep[0].size())
    print(rep[1].size())
    ...
    for b, head in enumerate(heads):
        ...
        for step, index in enumerate(root.order):
            indexes[step, b] = index
        trees.append(tree)
    rep = treegru.forward(inputs, indexes, trees)
    print(rep[0].size())
    print(rep[1].size())

    print("test forward OK")

    print("test DTTreeGRU：")
    treeGRU = DTTreeGRU(10, 20)
    rep = treegru.forward(inputs, indexes, trees)
    print(rep[0].size())
    print(rep[1].size())
