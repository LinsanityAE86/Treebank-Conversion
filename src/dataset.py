...
class Dataset(object):
    def __init__(self, file_name, max_bucket_num=80, word_num_one_batch=5000, sent_num_one_batch=200,
        ...
        if "ud" in self._file_name_short:
            self._domain_id=1
            print("self._domain_id",self._domain_id)
        elif "vndt" in self._file_name_short:
            self._domain_id=2
            print("self._domain_id",self._domain_id)
        # elif "ud" in self._file_name_short:
        #     self._domain_id=3
        #     print("self._domain_id",self._domain_id)
                ...
                if len(line) == 0:
                    ...
                    if length >= min_len and (max_len < 0 or length <= max_len):
                        if "unlabel" in self._file_name_short:
                            self._instances.append(UnInstance(len(self._instances), lines, self._domain_id))#yli 19-5-16
        ...
        if self._use_bucket:
            ...
            for (i, max_len) in enumerate(max_len_buckets):
                ...
                batch_num_to_provide = max(1, round(float(inst_num) * max_len / self._word_num_one_batch), math.ceil(float(inst_num)/self._sent_num_one_batch))
    ...
    def get_one_batch(self, rewind=False):
        self.one_batch = []
