...
def find_pattern_paths(argument_path, predicate_path):
    ...
    for i, a in enumerate(argument_path):
        ...
        if is_find is True:
            ...
            if pattern is None:
                print
                "pattern not found", argument_path, predicate_path
                exit()
...
if __name__ == '__main__':
    # 先创建树
    root, tree = creatGRUTree([-1, 2, 0, 2, 2])
    # 假设谓词位于索引11，论元位于索引5
    predicate_index = 1
    argument_index = 3

    # 获取从谓词到根的路径和从论元到根的路径
    predicate_to_root_path = tree[predicate_index].traverse_to_root()
    argument_to_root_path = tree[argument_index].traverse_to_root()
    print("谓词到根的路径:", predicate_to_root_path)
    print("论元到根的路径:", argument_to_root_path)

    # 测试find_sub_paths函数
    sub_argument_path, sub_predicate_path = find_sub_paths(argument_to_root_path, predicate_to_root_path)
    print("共同子路径:")
    print("论元子路径:", sub_argument_path)
    print("谓词子路径:", sub_predicate_path)

    # 测试find_pattern_paths函数
    pattern, common_ancestor, sub_argument_path, sub_predicate_path = find_pattern_paths(sub_argument_path,
                                                                                         sub_predicate_path)
    print("模式:", pattern)
    print("共同祖先:", common_ancestor)
    print("论元子路径:", sub_argument_path)
    print("谓词子路径:", sub_predicate_path)

    # 测试find_sentence_sub_paths函数
    sentence_sub_paths = find_sentence_sub_paths(tree, predicate_index)
    print("句子子路径:", sentence_sub_paths)
