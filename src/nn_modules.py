...
class CharLSTM(torch.nn.Module):
    ...
    def forward(self, chars, chars_lengths):
       ...
       # sorted_chars = torch.tensor(sorted_chars).to(torch.int64)
        sorted_chars = torch.as_tensor(sorted_chars,dtype=torch.int64) #格式问题
...
class InputLayer(nn.Module):
    def __init__(self, name, conf, word_dict_size, ext_word_dict_size, char_dict_size, tag_dict_size, label_dict_size,
        ...
        # self._label_embed = nn.Embedding(label_dict_size, conf.label_emb_dim, padding_idx=padding_id)
        ...
        # self._label_embed = nn.Embedding(label_dict_size, conf.label_emb_dim, padding_idx=padding_id)
        # label_emb_init = np.zeros((label_dict_size, conf.label_emb_dim), dtype=data_type)  # normal distribution
        # self._label_embed.weight.data = torch.from_numpy(label_emb_init)

        self.src_label_embed = nn.Embedding(label_dict_size, conf.src_label_emb_dim, padding_idx=padding_id)
        src_label_emb_init = np.zeros((label_dict_size, conf.src_label_emb_dim), dtype=data_type)  # normal distribution
        self.src_label_embed.weight.data = torch.from_numpy(src_label_emb_init)
        ...
        print('char={}'.format(char_dict_size))
        print('label23={}'.format(label_dict_size))
    ...
    def forward(self, words, ext_words, tags,  domains, word_lens_encoder, char_idxs_encoder, subword_idxs, subword_masks, token_starts_masks, sens, cuda_device):
        #words = torch.tensor(words).to(torch.int64)
        words = torch.as_tensor(words, dtype=torch.int64)
        ...
        if (x_ext_word_embed.size()[1]<x_word_embed.size()[1]):
            pad = torch.zeros(x_word_embed.size()[0],x_word_embed.size()[1]-bert_outs.size()[1],x_word_embed.size()[2])#.cuda(cuda_device)
