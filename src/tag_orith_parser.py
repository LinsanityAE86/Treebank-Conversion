...
class Parser(object):
    ...
    def init_models(self):
        ...
        self._input_layer = InputLayer('input', self._conf, self._word_dict.size(),
                                       self._ext_word_dict.size(), self._char_dict.size(), self._label_dict.size(), self._ext_word_emb_np)
    ...
    def forward(self, words, ext_words, lstm_masks, domains, domain_id, word_lens, chars_i, unlabel=False):
        ...
        if self._conf.is_shared_lstm:
            ...
            private_lstm_out=self._lstm_layer[domain_id](input_out, lstm_masks, initial=None, is_training=is_training)#[10,544,400]
        ...
        if unlabel == False:
            ...
            if (self._conf.is_adversary and self._conf.is_shared_lstm):
                classficationd, pos, nadv_class= self.classfication_module(shared_lstm_out, private_lstm_out_tgt, lstm_out)
    ...
    def train_or_eval_one_batch(self, dataset, is_training, unlabel = False):
        ...
        if unlabel == False:
            ...
            if (self._conf.is_adversary and self._conf.is_shared_lstm):
                arc_scores, label_scores, classficationd, pos, diff, nadv_class = self.forward(words, ext_words, lstm_masks, domains, dataset.domain_id, word_lens, chars_i)
            elif self._conf.is_adversary:
                arc_scores, label_scores, classficationd, pos, nadv_class = self.forward(words, ext_words, lstm_masks, domains, dataset.domain_id, word_lens, chars_i)
            elif self._conf.is_shared_lstm:
                arc_scores, label_scores, diff = self.forward(words, ext_words, lstm_masks, domains, dataset.domain_id, word_lens, chars_i)
            else:
                arc_scores, label_scores = self.forward(words, ext_words, lstm_masks, domains, dataset.domain_id, word_lens, chars_i)
        #print("onebatch len:",len(one_batch))
            ...
            if self._conf.is_adversary:
                adv_loss, tag_loss, nadv_loss = Parser.adversary_loss(classficationd, nadv_class, domains, domains_nadv, pos, tags, total_word_num)
            ...
            classficationd, pos, diff, nadv_class = self.forward(words, ext_words, lstm_masks, domains, dataset.domain_id, word_lens, chars_i, unlabel)
