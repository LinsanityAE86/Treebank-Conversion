...
class Parser(object):
    ...
    def add_lstm(self, i, lstm_input_size):  #i用来区分不同的LSTM层，size表示输入的特征维度
    ...
    def init_models(self):
        ...
        self._input_layer = InputLayer('input', self._conf, self._word_dict.size(), self._ext_word_dict.size(),\
                self._char_dict.size(), self._tag_dict.size(), self._label_dict(), self._ext_word_emb_np)
        ...
        if self._conf.is_shared_lstm:
            for k in range(4):#self._conf.domain_size):#when we uses all domains,we can change 1 to self._conf.domain_size
                if k<2:
                    self.add_lstm(k, lstm_input_size)#_domain)#lstm_input_size + 2 * self._conf.lstm_hidden_dim)
                else:
                    self.add_lstm(k, lstm_input_size)
            if self._conf.is_gate_lstm:
        ...
        if self._conf.is_adversary:
            ...
            #self._classficationD.append(Pos('postag', activation=nn.Tanh(),\
            #        input_size= 4 *self._conf.lstm_hidden_dim, hidden_size=self._tag_dict.size()))
    ...
    def forward(self, words, ext_words, tags, lstm_masks, domains, domain_id, word_lens, chars_i, unlabel=False):
        ...
        if self._conf.is_shared_lstm:
            shared_lstm_out=self._lstm_layer[0](input_out, lstm_masks, initial=None, is_training=is_training)#[10,544,400]
            private_lstm_out=self._lstm_layer[1](input_out, lstm_masks, initial=None, is_training=is_training)#[10,544,400]
            adv_lstm_out1=self._lstm_layer[2](input_out, lstm_masks, initial=None, is_training=is_training)#[10,544,400]
            domain_lstm_out1=self._lstm_layer[3](input_out, lstm_masks, initial=None, is_training=is_training)#[10,544,400]
            #adv_lstm_out1 = self._linear[0](shared_lstm_out)
            #domain_lstm_out1 = self._linear[1](private_lstm_out)
            #adv_lstm_out1 = self._linear[0](adv_lstm_out)
            #domain_lstm_out1 = self._linear[1](domain_lstm_out)
            ...
            y_pri  = F.softmax(nadv_class,dim = 2)
            y_pri = (private_lstm_out.unsqueeze(-1)@y_pri.unsqueeze(-2)).sum(-1) 
            shared_lstm_out = y_shared  
            private_lstm_out = y_pri
            #classficationd, nadv_class = self.classfication_module(shared_lstm_out, private_lstm_out)
            #if self._conf.is_gate_lstm:
                #shared_lstm_out = self._gate_lstm[0](adv_lstm_out1, shared_lstm_out)
                #private_lstm_out = self._gate_lstm[1](domain_lstm_out1, private_lstm_out)
                #shared_lstm_out =  
                #private_lstm_out = domain_lstm_out1
                #shared_lstm_out = adv_lstm_out1
                #private_lstm_out = domain_lstm_out1
        ...
        if unlabel == False:
            ...
            #classficationd, nadv_class = self.classfication_module(shared_lstm_out, private_lstm_out)
            return classficationd, diff, nadv_class
    ...
    def train_set_label(self,bc,pb,zx,domain, domain_src, domain_tgt, dataset, unlabel):
        if (domain_tgt < bc):
            domain =0
            ...
            domain = 2
        inst_num, loss = self.train_or_eval_one_batch(self._train_datasets[domain], is_training=True)#, unlabel=True)
        domain_tgt += 1
        return domain_src, domain_tgt, domain, inst_num, loss
    ...
    def train(self):
        ...
        domain, domain_src, domain_tgt, train_iter, udomain, udomain_src, udomain_tgt, u_cnt, l_cnt = 0, 0, 0, 0, 1, 0, 0, 0, 0
        while True:
            ...
            '''
            ubc,upc,upb,uzx=self._unlabel_train_datasets[0].batch_num,self._unlabel_train_datasets[1].batch_num, self._unlabel_train_datasets[2].batch_num,self._unlabel_train_datasets[3].batch_num
            if eval_cnt < 11:
                if u_cnt <2684:
                    udomain_src, udomain_tgt, udomain, inst_num, loss, parser, advmodel = self.train_set(ubc, upc, upb, uzx, parser, advmodel, udomain, udomain_src, udomain_tgt, self._unlabel_train_datasets, unlabel=True)
                    u_cnt +=1
                    print("udomain:", udomain)
                    print("u_cnt:", u_cnt)
                else:
                    domain_src, domain_tgt, domain,inst_num, loss, parser, advmodel = self.train_set_label(bc, pc, pb, zx, parser, advmodel, domain, domain_src, domain_tgt, self._train_datasets, unlabel=False)
                    l_cnt +=1
                    print("domain:", domain)
                    print("l_cnt:", l_cnt)
    ...
    def train_or_eval_one_batch(self, dataset, is_training, unlabel = False):
        ...
        if unlabel == False:
            ...
            if (self._conf.is_adversary and self._conf.is_shared_lstm):
                ...
                arc_scores, label_scores, classficationd, nadv_class = self.forward(words, ext_words, tags, lstm_masks, domains, dataset.domain_id, word_lens, chars_i)
            elif self._conf.is_shared_lstm:
                arc_scores, label_scores, diff = self.forward(words, ext_words, tags, lstm_masks, domains, dataset.domain_id, word_lens, chars_i)
            else:
                arc_scores, label_scores = self.forward(words, ext_words, tags, lstm_masks, domains, dataset.domain_id, word_lens, chars_i)
        #print("onebatch len:",len(one_batch))
            ...
            if self._conf.is_adversary:
                ...
                #ClassificationD.compute_accuray(classficationd, domains_nadv)
                print("nadversary loss:",nadv_loss)
                ClassificationD.compute_accuray(nadv_class, domains_nadv)
            ...
            classficationd, diff, nadv_class = self.forward(words, ext_words, tags, lstm_masks, domains, dataset.domain_id, word_lens, chars_i, unlabel)
